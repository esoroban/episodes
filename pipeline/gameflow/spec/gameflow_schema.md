...
- `constructor` — пошаговая сборка ответа из выбора по шагам. Поля сцены: `attack` (опционально — реплика собеседника, на которую отвечаем), `intro` (контекст/цель), `steps[]` (каждый — `id`, `label`, `hint`, `options[]`), `template` (как склеить пики, синтаксис `{step_id}`), `closing` (разбор Софы). У каждой опции — свой `fb` для именно этой ошибки/успеха. Финальный ответ собирается из `correct: true` пиков по `template` и эмитится одним пузырём Софы.
  ...
  **Два режима:**
  - **Repair-mode** — игрок собирает ВОЗРАЖЕНИЕ против чужой нападки. `attack` заполнен (фраза собеседника). Цель — построить этичный ответ. См. ep_041 s23 (4-step), ep_041 s24/s25 (2-step), ep_042 s22/s23 (признание частичной правоты), ep_044 s13/s14/s27 (универсальный ответ).
  - **Drill-mode** — игрок собирает ДЕМАГОГИЧЕСКУЮ фразу САМ, чтобы узнать приём изнутри. `attack` пустой (нет входящей реплики). `intro` содержит цель манипуляции. `hint` каждого шага прямо говорит игроку, какую опцию выбрать (демагогическую). Неправильные опции — настоящий аргумент / возражение / нейтральная фраза — оставлены, чтобы игрок видел разницу. После каждой собранной фразы Софа в `closing` разбирает: что сработало, почему это манипуляция. См. ep_041 s04_d1..s04_d9 (9 канонических демагогических приёмов перед сценой Марко).
